...
\subsubsection{Cloud Pub/Sub API} Cloud Pub/Sub API is a message
...
\subsubsection{Cloud Vision API} Cloud Vision API is the most popular
...
\subsubsection{Initial Setup} As mentioned above, the application
...
\subsubsection{Mainapp Microservice} The mainapp provides the actual
  ...
  the service is briefly discussed.  The $yelp\_images.py$ has 4
  functions $query\_api()$, $get\_business()$, $search()$ and $request()$; As
  the given location and term are passed to $query\_api$, it sends a
  ...
  $get\_business()$ is called to retrieve business details. From the
...
\subsubsection{Pods, Services, Deployments} There is one yaml file for
